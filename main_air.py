...
from diffusion_net.utils import set_device,LpLoss,MultipleLoss,convert_to_one_hot,toNP,CarCFDLoss
from torch.utils.data import DataLoader
from glno_dataset import AIRFOILBase
import yaml
...
torchrun --nproc_per_node=3 --master_addr=127.0.0.1 --master_port=29507 main_air.py --input_features xyz

python main_air.py --input_features xyz
'''
...
dtype = torch.float32
with open('config_air.yaml', 'r', encoding='utf-8') as f:
...
op_cache_dir = os.path.join(base_path, "data","airfoil", "op_cache")
...
model_save_path = os.path.join(base_path,
                               "data/airfoil/saved_models/airfoil{}_{}.pth".format(input_features, save_index))
dataset_path = os.path.join(base_path, "data/airfoil")

# === Load datasets

# process_path = dataset_path + '/processed-004.pt'
# print('Loading processed data from ', process_path)
# train_data, valid_data, test_data = torch.load(process_path)
# test_dataset = AIRFOILBase(data=test_data, root_dir=dataset_path, mode='test', k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
# train_dataset = AIRFOILBase(data=train_data, root_dir=dataset_path, mode='train', k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
# valid_dataset = AIRFOILBase(data=valid_data, root_dir=dataset_path, mode='val',k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
    
# === Load datasets

if not args.evaluate: 
    train_dataset = AIRFOILBase(root_dir=dataset_path, mode='train', k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
    if args.distributed:
        train_sampler = DistributedSampler(train_dataset, num_replicas=args.world_size, rank=args.rank, shuffle=True)
    else:
        train_sampler = None
    train_loader = DataLoader(train_dataset, batch_size=10, sampler=train_sampler, shuffle=(train_sampler is None))

test_dataset = AIRFOILBase(root_dir=dataset_path, mode='test', k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
if args.distributed:
...
valid_dataset = AIRFOILBase(root_dir=dataset_path, mode='val',k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
if args.distributed:
    valid_sampler = DistributedSampler(valid_dataset, num_replicas=args.world_size, rank=args.rank, shuffle=False)
else:
    valid_sampler = None
valid_loader = DataLoader(valid_dataset, batch_size=16, sampler=valid_sampler, shuffle=(train_sampler is None))
...
C_in = {'xyz': 6, 'hks': 16}[input_features]  # dimension of input features
...
def train_epoch(epoch):
    ...
    # 收集所有进程的损失
    if args.distributed:
        losses_tensor = torch.tensor([np.mean(losses1), np.mean(losses2)], device=device)
        dist.all_reduce(losses_tensor, op=dist.ReduceOp.SUM)
        train_loss = losses_tensor / args.world_size
    else:
        train_loss = [np.mean(losses1),np.mean(losses2),np.mean(losses3),np.mean(losses4)]
...
if train:
    ...
    for epoch in range(n_epoch):
        ...
        if args.rank==0:
            print("Epoch {} - Train L2: {:.3e} {:.3e} {:.3e} {:.3e}\n Val L2: {:.3e} {:.3e} {:.3e} {:.3e}".format(epoch, train_l2loss[0],train_l2loss[1],train_l2loss[2],train_l2loss[3],\
                                                                                                                  val_l2loss[0],val_l2loss[1],val_l2loss[2],val_l2loss[3]))
