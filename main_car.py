...
from glno_dataset import CarCFDBase
import yaml
...
torchrun --nproc_per_node=1 --master_addr=127.0.0.1 --master_port=29507 main_car.py --input_features xyz

python main_car.py --input_features xyz
'''
...
dtype = torch.float32
with open('config_car.yaml', 'r') as f:
...
op_cache_dir = os.path.join(base_path, "data","car", "op_cache")
...
model_save_path = os.path.join(base_path,
                               "data/car/saved_models/car_{}_{}.pth".format(input_features, save_index))
dataset_path = os.path.join(base_path, "data/car")

# === Load datasets

# process_path = dataset_path + '/processed.pt'
# print('Loading processed data from ', process_path)
# train_data, valid_data, test_data = torch.load(process_path)
# if args.rank == 0:
#     train_dataset = CarCFDBase(data=train_data, root_dir=dataset_path, mode='train', k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
# if args.rank==1:
#     test_dataset = CarCFDBase(data=test_data, root_dir=dataset_path, mode='test', k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
# if args.rank==2:
#     valid_dataset = CarCFDBase(data=valid_data, root_dir=dataset_path, mode='val',k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
    
# === Load datasets

if not args.evaluate: 
    train_dataset = CarCFDBase(root_dir=dataset_path, mode='train', k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
    if args.distributed:
...
test_dataset = CarCFDBase(root_dir=dataset_path, mode='test', k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
if args.distributed:
    test_sampler = DistributedSampler(test_dataset, num_replicas=args.world_size, rank=args.rank, shuffle=False)
else:
    test_sampler = None
test_loader = DataLoader(test_dataset, batch_size=10, sampler=test_sampler, shuffle=False)

valid_dataset = CarCFDBase(root_dir=dataset_path, mode='val',k_eig=k_eig, use_cache=True, op_cache_dir=op_cache_dir)
if args.distributed:
    valid_sampler = DistributedSampler(valid_dataset, num_replicas=args.world_size, rank=args.rank, shuffle=False)
else:
    valid_sampler = None
valid_loader = DataLoader(valid_dataset, batch_size=10, sampler=valid_sampler, shuffle=False)
...
C_in = {'xyz': 7, 'hks': 16}[input_features]  # dimension of input features
...
myloss = CarCFDLoss(size_average=True)
...
def train_epoch(epoch):
    ...
    if args.distributed:
        ...
        train_loss = [np.mean(losses1),np.mean(losses2)]
...
if train:
    ...
    for epoch in range(n_epoch):
        ...
        if args.rank==0:
            print("Epoch {} - Train L2: {:.3e},{:.3e} Val L2: {:.3e},{:.3e}".format(epoch, train_l2loss[0],train_l2loss[1], val_l2loss[0],val_l2loss[1]))
        if epoch % 50 == 0:
